<div class="relative">
        ...
        <div class="grid grid-cols-3 justify-between">
            ...
            <div class="grid grid-cols-8">
                <div class="w-7 col-start-5">
                    <div id="open-overlay" class="cursor-pointer">
    ...
    <div class="w-full h-72 md:h-screen overflow-hidden relative">
        ...
        <div class="flex h-full transition-transform duration-500 ease-in-out transform relative">
            ...
            <div class="w-full h-full flex-shrink-0">
                <img src="https://picsum.photos/200" class="w-full h-full object-cover" alt="...">
            </div>
            <!-- Item 2 -->
            <div class="w-full h-full flex-shrink-0">
                <img src="https://picsum.photos/200" class="w-full h-full object-cover" alt="...">
    ...
    <div class="flex absolute bottom-5 left-1/2 z-30 space-x-3 -translate-x-1/2">
        <button type="button"
            ...
            aria-current="false" aria-label="Slide 1" onclick="changeSlide(0)"></button>
        <button type="button"
            class="w-3 h-3 rounded-full bg-[#D9D9D9] transform scale-100 transition-transform duration-300"
            aria-current="false" aria-label="Slide 2" onclick="changeSlide(1)"></button>
        <button type="button"
            class="w-3 h-3 rounded-full bg-[#D9D9D9] transform scale-100 transition-transform duration-300"
            aria-current="false" aria-label="Slide 3" onclick="changeSlide(2)"></button>
        <button type="button"
            class="w-3 h-3 rounded-full bg-[#D9D9D9] transform scale-100 transition-transform duration-300"
            aria-current="false" aria-label="Slide 4" onclick="changeSlide(3)"></button>
        <button type="button"
            class="w-3 h-3 rounded-full bg-[#D9D9D9] transform scale-100 transition-transform duration-300"
            aria-current="false" aria-label="Slide 5" onclick="changeSlide(4)"></button>
    </div>

    <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 transition duration-300 ease-out transform translate-y-full overlay hidden"
        id="overlay">
        <div class="bg-white p-8 rounded-lg">
            <!-- Konten overlay di sini -->
            <button id="close-overlay" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700">Close
                Overlay</button>
        </div>
    </div>
    ...
    <script>
        ...
        let storedIndex = 0; // Variable to store the current index before opening the overlay
        ...
        openOverlayBtn.addEventListener('click', () => {
            storedIndex = currentIndex; // Store the current index before opening the overlay
            overlay.classList.remove('hidden');
            overlay.classList.remove('translate-y-full');
            overlay.classList.add('animation-enter-active', 'animation-enter-from');

            setTimeout(() => {
                overlay.classList.remove('animation-enter-from');
                overlay.classList.add('animation-enter-to');
            }, 0);
        });

        closeOverlayBtn.addEventListener('click', () => {
            overlay.classList.remove('animation-enter-to');
            overlay.classList.add('animation-leave-active', 'animation-leave-from');

            setTimeout(() => {
                overlay.classList.remove('animation-leave-from');
                overlay.classList.add('animation-leave-to');
            }, 0);

            setTimeout(() => {
                overlay.classList.add('hidden');
                changeSlide(storedIndex); // Set the carousel back to the stored index
